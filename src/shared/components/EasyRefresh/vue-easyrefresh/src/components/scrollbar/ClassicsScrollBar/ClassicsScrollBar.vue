<template>
  <div class="er-classic-scrollbar">
    <transition name="scroll-bar-fade">
      <div
        v-if="showBar"
        class="er-classic-scrollbar-background"
        :style="`width: ${width}px;${position !== 'left' ? 'right: 0;' : ''}background: ${bgColor};padding: 0 ${padding}px;`"
      >
        <div
          class="er-classic-scroll-bar"
...
.er-classic-scrollbar {
  ...
  .scroll-bar-fade-enter-active,
  .scroll-bar-fade-leave-active {
    transition: opacity 0.5s;
  }
  .scroll-bar-fade-enter,
  .scroll-bar-fade-leave-active {
    opacity: 0;
  }
}
