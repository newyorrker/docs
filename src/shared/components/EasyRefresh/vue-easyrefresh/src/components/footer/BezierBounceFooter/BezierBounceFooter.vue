<template>
  <div class="er-bezier-bounce-footer" :style="'height: ' + footerHeight + 'px;'">
    ...
    <div class="ball-box">
      <div
        class="ball"
        :style="`background: ${color};margin-left: ${ballMargin}px;margin-right: ${ballMargin}px;opacity: ${ballOpacity * 0.5};`"
      ></div>
      <div
        class="ball"
        :style="`background: ${color};margin-left: ${ballMargin}px;margin-right: ${ballMargin}px;opacity: ${ballOpacity * 0.75};`"
      ></div>
      <div
        class="ball"
        :style="`background: ${color};margin-left: ${ballMargin}px;margin-right: ${ballMargin}px;opacity: ${ballOpacity};`"
      ></div>
      <div
        class="ball"
        :style="`background: ${color};margin-left: ${ballMargin}px;margin-right: ${ballMargin}px;opacity: ${ballOpacity * 0.75};`"
...
export default class BezierBounceFooter extends Vue implements Footer {
  ...
  private drawCanvas() {
    ...
    this.drawBackground();
    if (
      (this.footerStatus === FooterStatus.NO_LOAD || this.footerStatus === FooterStatus.LOAD_READY) &&
      this.footerHeight >= this.defaultHeight
    ) {
      this.drawBackground();
      this.drawPullRadian();
  ...
  private drawPullRadian() {
    ...
    this.canvas.quadraticCurveTo(
      ...
      this.canvasDom.height - this.defaultHeight,
    );
    this.canvas.fill();
  }
  // 绘制回弹弧度
