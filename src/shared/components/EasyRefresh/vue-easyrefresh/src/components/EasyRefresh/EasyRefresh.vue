...
import ClassicsScrollBar from '../scrollbar/ClassicsScrollBar/ClassicsScrollBar.vue';
...
export default class EasyRefresh extends Vue {
  ...
  public callLoadMore() {
    ...
    this.scroller.triggerPushToLoad(
      this.footer.loadHeight() + 30,
      () => {
        this.scroller.triggerPushToLoad(
          this.footer.loadHeight(),
          () => {
  ...
  private updateScrollBar(top: number) {
    ...
    if (this.container!!.clientHeight > this.content!!.offsetHeight) {
      ...
      exceededLength = -top;
      scrollBarLength = (this.container!!.clientHeight * this.container!!.clientHeight) / (this.content!!.offsetHeight + exceededLength);
      topOffset = 0;
      scrollBarScale = 0;
      ...
      // exceededLength = 0
      scrollBarLength = (this.container!!.clientHeight * this.container!!.clientHeight) / this.content!!.offsetHeight;
  ...
  private scrollerCallBack(left: number, top: number, zoom: number) {
    ...
    if (top < 0) {
      ...
      } else if (this.headerStatus === HeaderStatus.REFRESH_READY && -top < this.header.refreshHeight() && this.userScrolling) {
        // 刷新恢复
        this.header.onRefreshRestore();
        this.changeHeaderStatus(HeaderCallBackStatus.RESTORE);
        this.headerStatus = HeaderStatus.REFRESH_START;
      }
    } else if (top > 0 && top > scrollableDistance) {
      // 显示Footer
      if (!this.loadMore) {
        return;
      }
      ...
      if (this.footerStatus === FooterStatus.NO_LOAD && this.userScrolling) {
        // 开始加载
        this.footer.onLoadStart();
      ...
      } else if (this.footerStatus === FooterStatus.LOAD_READY && footerHeight < this.footer.loadHeight() && this.userScrolling) {
        // 恢复加载
  ...
  private scrollActionEnd(e: UIEvent) {
    ...
    if (this.onRefresh) {
      ...
      const { left, top, zoom } = this.scroller.getValues();
      // 触发刷新
      if (this.headerStatus === HeaderStatus.REFRESH_READY && -top > this.header.refreshHeight()) {
    ...
    if (this.loadMore && !this.noMore) {
      ...
      if (
        this.footerStatus === FooterStatus.LOAD_READY &&
        ((this.content!!.clientHeight >= this.container!!.clientHeight && top - scrollableDistance >= this.footer.loadHeight()) ||
          (this.content!!.clientHeight < this.container!!.clientHeight && top >= this.footer.loadHeight()))
      ) {
        this.scroller.doTouchEnd(e.timeStamp, true);
        this.scroller.triggerPushToLoad(
          this.footer.loadHeight(),
...
.v-easy-refresh {
  ...
  .v-easy-refresh-body {
    ...
    left: 0;
    overflow: hidden;
    .v-easy-refresh-float-header-offset,
    .v-easy-refresh-float-footer-offset {
      width: 100%;
    }
  }
