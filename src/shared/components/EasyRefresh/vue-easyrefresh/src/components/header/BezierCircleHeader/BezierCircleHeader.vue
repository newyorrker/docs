...
export default class BezierCircleHeader extends Vue implements Header {
  ...
  private drawCircleRebound() {
    // 计算圆圈的值
    this.reboundCircleValue =
      this.reboundValue <= this.defaultHeight * 2
  ...
  private drawWater() {
    ...
    if (this.reboundWaterValue < this.circleR) {
      ...
      this.canvas.quadraticCurveTo(curveX + this.circleR - width / 2, curveY, this.canvasDom.width / 2 - this.circleR, this.defaultHeight);
      this.canvas.lineTo(this.canvasDom.width / 2 + this.circleR, this.defaultHeight);
      this.canvas.quadraticCurveTo(
        curveX + width - (this.circleR - width / 2),
        curveY,
        this.canvasDom.width / 2 + this.circleR,
        this.defaultHeight - this.reboundWaterValue,
      );
      this.canvas.fill();
